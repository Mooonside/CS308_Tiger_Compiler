...
public class CodeGen {
	...
	{
		//翻译标记,注意label和name的区别
		//label用以标记下一条指令的地址(PC),而name则用在跳转中需要调用label的地址值时使用
	...
	private void munchStm(Tree.CJUMP j) {
		...
		emit(new Assem.OPER(oper + " `s0, `s1, `j0", 
							null, L(t1, L(t2, null)), new Temp.LabelList(j.iftrue, new Temp.LabelList(j.iffalse, null))));	
	}	
	...
	private Temp.Temp munchExp(Tree.BINOP b){
		...
		if (b.right instanceof Tree.CONST){//常数在右子树
		...
		{//当常数出现在左子树时不能简单地将左右子树的位置互换，可以将左子树的内容放入寄存器
	...
	private Temp.Temp munchExp(Tree.CALL c) {
		...
		for (Tree.ExpList a = c.args; a != null; a = a.tail, i++) {
			...
			if (a.head instanceof Tree.CONST)
				//参数为常数
				emit(new Assem.OPER("li $a" + i + ", " +
						((Tree.CONST) a.head).value,null,null));
			else{
				//参数为寄存器
				t = munchExp(a.head);
		...
		//跳转到该函数地址
		emit(new Assem.OPER("jal " + ((Tree.NAME) c.func).label, 
				((MipsFrame)frame).callerSaves, list));
		return frame.RV();
	}
}
